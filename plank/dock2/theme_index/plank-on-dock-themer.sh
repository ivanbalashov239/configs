...
cd ~/.config/plank/dock2/theme_index/
ls
_dest="$HOME/.config/plank/dock2/theme_index/"
...
while IFS= read -r file
		...
		done < "$HOME/.config/plank/dock2/theme_index/themes.list"
...
#==================================
	#theme lists
	mkdir -p $HOME/.config/plank/dock2/theme_index/
	if [ -f $HOME/.config/plank/dock2/theme_index/themes.list  ]; then rm $HOME/.config/plank/dock2/theme_index/themes.list ; fi 
	#delete files if exist
	ls /usr/share/plank/themes > $HOME/.config/plank/dock2/theme_index/themes.list  
	echo
	echo
	echo "Yeah. I see. You have these themes installed:"
	cat $HOME/.config/plank/dock2/theme_index/themes.list
	echo
	echo
...
#		loc="$HOME/.config/plank/dock2/theme_index/theme-icons"
#				echo "Testing for previous installs...." #info on previous versions
#				if [ -d "$loc" ]; then #-d check directories
#					echo "Previous icon FOLDER Detected! ...Automatically erasing that stuff"
#					rm -R "$HOME/.config/plank/dock2/theme_index/plank-themer-ondock"
#				else
#				echo "...no previous configs found:"
#				echo "Creating a new stuff"
#			fi
#		mkdir -p $HOME/.config/plank/dock2/theme_index/theme-icons
#wget  -O $HOME/.config/plank/dock2/theme_index/theme-icons/plank_theme.svg
#
...
while IFS= read -r file
		...
		echo Icon="$HOME/.config/plank/dock2/theme_index/theme-icons/plank_theme.svg" >> "$file".desktop
...
while IFS= read -r file
		...
		echo sed -i \'s/Theme=.*$/Theme=$file/g\' ~/.config/plank/dock2/settings >> "$_dest""$file".sh
		echo "--------------------------------"
ls "$_dest"
cat "$_dest""$file".sh
echo
...
#====================================
chmod +x $HOME/.config/plank/dock2/theme_index/*.desktop 
chmod +x $HOME/.config/plank/dock2/theme_index/*.sh

#MOVING THE .DESKTOP FILES TO THE PROPER TEMP LOCATION
#======================================================
rm -R $HOME/.config/plank/dock2/theme_index/plank-themer-ondock;
mkdir -p $HOME/.config/plank/dock2/theme_index/plank-themer-ondock
cp -a $HOME/.config/plank/dock2/theme_index/*.desktop $HOME/.config/plank/dock2/theme_index/plank-themer-ondock
...
cd $HOME/.config/plank/dock2/theme_index/
...
echo ---------------------------------
		...
		echo Icon="$HOME/.config/plank/dock2/theme_index/updater-icon/update-theme-list.svg" >> "-Update-theme-list.desktop"
		echo Exec="$HOME/.config/plank/dock2/theme_index/plank-on-dock-themer-update-theme-list".sh >> "-Update-theme-list.desktop"
		echo "--------------------------------"
...
#CHECK INSTALL... final result on actual plank dockitems folder
		if [ -f "$HOME/.config/plank/dock2/launchers/plank-themer-ondock.dockitem" ]; then
			echo "old plank-themer-ondock.dockitem Detected! ...Automatically erasing that stuff"
			rm "$HOME/.config/plank/dock2/launchers/plank-themer-ondock.dockitem"
		else
...
#		if [ -f "$HOME/.config/plank/dock2/theme_index/plank-themer-ondock.dockitem" ]; then
#			echo "old plank-themer-ondock.dockitem Detected! ...Automatically erasing that stuff"
#			rm "$HOME/.config/plank/dock2/theme_index/plank-themer-ondock.dockitem"
#		else
#		echo "plank-themer-ondock...no DOCK ITEM found:"
#		echo "Creating a brand new one"
#	fi
...
cd $HOME/.config/plank/dock2/theme_index/
touch $HOME/.config/plank/dock2/theme_index/plank-themer-ondock.dockitem
echo \[PlankItemsDockItemPreferences\] >> $HOME/.config/plank/dock2/theme_index/plank-themer-ondock.dockitem 
echo Launcher=file://$HOME/.config/plank/dock2/theme_index/plank-themer-ondock >> $HOME/.config/plank/dock2/theme_index/plank-themer-ondock.dockitem 

echo "Adding dockitem to the dock..."
#copying the aux file to the actual folder. Fixing result.
cp $HOME/.config/plank/dock2/theme_index/plank-themer-ondock.dockitem ~/.config/plank/dock2/launchers/ 
#FIX
#sed -i 's@Launcher=*@Launcher=file://$HOME/.config/plank/dock2/theme_index/plank-themer-ondock@' $HOME/.config/plank/dock2/launchers/plank-themer-ondock.dockitem 
#=======================
#DOCK ITEM CREATED
#=======================
...
#touch -d date -d @1387748732 $HOME/.config/plank/dock2/launchers/plank-themer-ondock.dockitem 

const='plank-themer-ondock.dockitem'
if [ -e ~/.config/plank/dock2/settings ]; then
	  grep -q -e "$const" ~/.config/plank/dock2/settings
  echo "dockitem already there, nothing to do"
  if [ $? = 1 ]; then
    echo "$const  ; not found but will be inserted as first item!"
    sed -i 's@DockItems=plank.dockitem;;@DockItems=plank.dockitem;;plank-themer-ondock.dockitem;;@' ~/.config/plank/dock2/settings 
	sed -i 's@DockItems=plank.dockitem;;@DockItems=plank.dockitem;;plank-themer-ondock.dockitem;;@' ~/.config/plank/dock2/settings 
	sed -i 's@plank-themer-ondock.dockitem\n@\n@g' ~/.config/plank/dock2/settings 
  fi
fi
...
cd $HOME/.config/plank/dock2/theme_index/plank-themer-ondock
cp $HOME/.config/plank/dock2/theme_index/*.desktop $HOME/.config/plank/dock2/theme_index/plank-themer-ondock
cd $HOME/.config/plank/dock2/theme_index/plank-themer-ondock
chmod u+x **/*.sh
...
confirm "Install the themes (y/Y) or finish the installation (n/N)"&& cd $HOME/.config/plank/dock2/themes-repo/Themes && sudo cp -v -a . /usr/share/plank/themes
#http://stackoverflow.com/questions/3231804/in-bash-how-to-add-are-you-sure-y-n-to-any-command-or-alias


#if [[ $response =~ ^(yes|y|Y|YES|Yes)$ ]];then
#    echo "copying"
#    cd $HOME/.config/plank/dock2/themes-repo/Themes && sudo cp -v -a . /usr/share/plank/themes
#fi
